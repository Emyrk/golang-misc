...
// Clients should consume from ReadFrom without blocking.
func (p *Peer) ReadFrom() (d []byte, remote net.Addr, broadcast bool, err error) {
	c := make(chan struct{})
	p.actions <- func() {
...
func (p *Peer) Write(b []byte) (n int, err error) {
	...
	p.actions <- func() {
		...
		pkt := pkt{SrcName: p.name, SrcUID: p.uid, Buf: b, Broadcast: true}
		// TODO(pb): detect and support broadcast
		buf := pkt.encode()
		n = len(buf)
		p.gossip.GossipBroadcast(pktSlice{pkt})
	}
	<-c
	return n, err
}
...
func (p *Peer) WriteTo(b []byte, dst net.Addr) (n int, err error) {
	...
	p.actions <- func() {
		...
		pkt := pkt{SrcName: p.name, SrcUID: p.uid, Buf: b, Broadcast: false}
